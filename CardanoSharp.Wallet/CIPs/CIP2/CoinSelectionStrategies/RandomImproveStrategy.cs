...
public class RandomImproveStrategy : BaseSelectionStrategy, IRandomImproveStrategy
{
    ...
    private bool CalculateCondition(Utxo v1, long ideal, long max, Utxo v0, int limit, ICollection utxos, Asset? asset)
    {
        ...
        arrayToMatchConditions[0] = asset is null
            ? Math.Abs((long)(ideal - (long)v1.Balance.Lovelaces)) < Math.Abs((long)(ideal - (long)(v0.Balance?.Lovelaces ?? 0)))
            : Math.Abs(
                (long)(ideal - v1.Balance.Assets.FirstOrDefault(x => x.PolicyId.SequenceEqual(asset.PolicyId) && x.Name.Equals(asset.Name))!.Quantity)
            )
                < Math.Abs(
                    (long)(
                        ideal
                        - (
                            v0.Balance?.Assets?.FirstOrDefault(x => x.PolicyId.SequenceEqual(asset.PolicyId) && x.Name.Equals(asset.Name))?.Quantity
                            ?? 0
                        )
                    )
                );
