...
public static class Bip32Utility
{
    ...
    public static byte[] Le32(uint i) => new byte[] { (byte)i, (byte)(i >> 8), (byte)(i >> 16), (byte)(i >> 24) };

    public static bool IsValidPath(string path) => !(path.Split('/').Slice(1).Select(a => a.Replace("'", "")).Any(a => !Int32.TryParse(a, out _)));

    public static DerivationType FromIndex(uint index) => index >= 0x80000000 ? DerivationType.HARD : DerivationType.SOFT;
