...
public static class UtxoExtensions
{
    ...
    {
        var inputArr = CBORObject.NewArray().Add(utxo.TxHash.HexToByteArray()).Add(utxo.TxIndex);

        var outputArr = CBORObject.NewArray().Add(new Address(utxo.OutputAddress).GetBytes());

        if (utxo.Balance.Assets != null && utxo.Balance.Assets.Count() > 0)
        {
            var assetArr = CBORObject.NewArray().Add(utxo.Balance.Lovelaces);
        ...
        return CBORObject.NewArray().Add(inputArr).Add(outputArr);
    }
    ...
    public static Utxo GetUtxo(this CBORObject utxoCbor)
    {
        ...
        {
            utxo.Balance.Assets = tempOutput.Value.MultiAsset
                .SelectMany(
                    x =>
